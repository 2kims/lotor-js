...
`.github/workflows/release.yml` denies permissions by default and separates release authority into four jobs:
...
1. Keep the existing `sdk-release` GitHub environment used by npm trusted publishing and restrict it to the selected branch `main`.
...
6. Store no secrets in `sdk-release`. Do not configure `NPM_TOKEN` or another token fallback.
...
8. Verify the environment, variable, and secret names without printing secret values:
   ...
   gh secret list --repo 2kims/lotor-js --env sdk-release
   gh api repos/2kims/lotor-js/environments/sdk-release
