...
export function verifyWorkflowSources({ pullRequest, release, packageJson, allWorkflows }) {
  ...
  invariant(!/(?:^|\s)[&*][A-Za-z0-9_-]+/m.test(pullRequest), "pull-request workflow must not use YAML aliases unsupported by actionlint");
  invariant(occurrences(pullRequest, /actionlint" -ignore 'label "blacksmith-2vcpu-ubuntu-2404" is unknown'/g) === 2,
    "both runner paths must lint workflows while ignoring only the approved custom runner label");
  ...
  invariant(!/\benvironment:/.test(releaseJob), "release automation must not enter the npm publishing environment");
